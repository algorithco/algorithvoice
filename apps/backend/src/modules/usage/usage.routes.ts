...
export async function usageRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (req) => {
      ...
      const days = Math.min(30, Math.max(1, parseInt(String(q.days ?? "7"), 10) || 7));
      ...
      for (const r of records) {
        ...
        if (!b) continue;
        const qty = typeof r.quantity === "number" ? r.quantity : (r.quantity as unknown as { toNumber: () => number }).toNumber();
        b.seconds += qty;
        b.requests += 1;
      }
      const out: { date: string; label: string; seconds: number; requests: number }[] = [];
      for (const [date, v] of buckets) {
        const label = new Date(date + "T00:00:00Z").toLocaleDateString("en-GB", { weekday: "short" }).slice(0, 3);
        out.push({ date, label, seconds: Math.round(v.seconds * 100) / 100, requests: v.requests });
      }
  ...
  app.get(
    ...
    async (req) => {
      ...
      const limit = Math.min(20, Math.max(1, parseInt(String(q.limit ?? "5"), 10) || 5));
      ...
      return {
        recent: rows.map((r) => ({
          id: r.id,
          metric: r.metric,
          seconds: typeof r.quantity === "number" ? r.quantity : (r.quantity as unknown as { toNumber: () => number }).toNumber(),
          model: r.model,
