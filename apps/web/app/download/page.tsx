...
async function getLatest(): Promise<GithubRelease | null> {
  const repo = process.env.NEXT_PUBLIC_GITHUB_REPO ?? "algorithco/algorithvoice-app";
...
function categorize(assets: GithubAsset[]) {
  const installers = assets.filter(
    (a) => /\.(dmg|msi|msix|exe|AppImage|deb|rpm)$/i.test(a.name) && !a.name.endsWith(".sig"),
  );
  const sigs = assets.filter((a) => a.name.endsWith(".sig"));
  const other = assets.filter((a) => !installers.includes(a) && !sigs.includes(a) && a.name !== "latest.json");
...
function PlatformIcon({ platform }: { platform: "windows" | "linux" | "mac" }) {
  if (platform === "windows") {
    return (
      <span className="grid size-10 place-items-center rounded-xl bg-[#0a0a0a] ring-1 ring-line" aria-hidden>
...
export default async function DownloadPage() {
  ...
  const repo = process.env.NEXT_PUBLIC_GITHUB_REPO ?? "algorithco/algorithvoice-app";
  const { installers, windows, linux, mac, sigs, latestJson } = rel ? categorize(rel.assets) : { installers: [] as GithubAsset[], windows: [], linux: [], mac: [], sigs: [], latestJson: null };
  ...
  const primaryLinux = linux.find((a) => /\.AppImage$/i.test(a.name)) ?? linux[0] ?? null;
  ...
  return (
    <div className="min-h-screen bg-canvas text-ink">
      ...
      <section className="relative -mt-20 overflow-hidden border-b border-line">
        <Aurora colorStops={["#ffffff", "#81738f", "#9a93b8"]} amplitude={1.0} blend={0.5} speed={0.9} opacity={0.45} />
        <div className="relative z-10 mx-auto max-w-[1200px] px-4 pt-36 pb-12 sm:px-6 sm:pt-40 md:px-16 md:pb-16">
          <Reveal>
            <div className="inline-flex items-center gap-2 rounded-full border border-line bg-surface/80 px-3 py-1.5 backdrop-blur">
              ...
              {version ? (
                <>
                  ...
                  <span className="hidden font-mono text-xs text-faint sm:inline">{version} · {formatDate(published)}</span>
                </>
              ) : null}
            </div>
            <h1 className="t-hero mt-6 max-w-[18ch] text-balance">Download Algorith Voice.</h1>
            <p className="t-lead mt-5 max-w-[60ch] text-pretty text-sub">
              Push-to-talk for Windows & Linux today — macOS Swift app coming soon. Every build is signed,
              verified with <span className="font-mono text-xs text-ink">latest.json</span> + <span className="font-mono text-xs text-ink">.sig</span> and
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              {hasRelease ? (
                ...
                <span className="t-cap text-faint">Checking GitHub releases… releases publish on version tags</span>
              )}
              <a href="/docs" className="inline-flex min-h-[40px] items-center rounded-full border border-line bg-canvas px-4 font-mono text-xs font-medium text-ink hover:bg-surface">
                View setup guide
              </a>
            </div>
          </Reveal>
          ...
          <Reveal delay={80}>
            <DownloadInteractive assets={installers} tag={version} publishedAt={published} />
          </Reveal>
        </div>
      </section>
          ...
          <div className="flex flex-wrap items-baseline justify-between gap-4">
            <div>
              ...
              <p className="t-body mt-3 max-w-[62ch] text-sub">
                Pick AppImage for portability or .deb for system install on Linux. Windows is a single NSIS
                installer. All binaries are built on CI with pinned Node + Rust.
              </p>
            </div>
            <div className="hidden items-center gap-2 sm:flex">
              <span className="h-2 w-2 rounded-full bg-emerald-500" />
              <span className="font-mono text-xs text-faint">{installers.length} files · {version ?? "—"}</span>
            </div>
          </div>
        </Reveal>
          ...
          <Reveal delay={60} className="min-w-0">
            <article className="group flex flex-col rounded-2xl border border-line bg-surface p-5 shadow-[0_8px_24px_rgba(0,0,0,0.2)] transition-all duration-200 hover:bg-raised sm:p-6">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="rounded-full border border-line bg-canvas px-2 py-1 font-mono text-[11px] font-medium text-faint">x64 · NSIS</span>
              </div>
              <h3 className="t-h2 mt-3 !text-[22px] sm:!text-[24px]">Windows</h3>
              <p className="mt-1 font-mono text-xs leading-4 text-faint">Windows 10 1809+ · WebView2 auto-installed</p>
              {primaryWindows ? (
                <>
                  <div className="mt-4 flex items-center justify-between gap-3 rounded-xl bg-canvas px-3 py-2.5 ring-1 ring-line">
                    <span className="min-w-0 truncate font-mono text-xs font-medium text-ink">{primaryWindows.name.replace("Algorith.Voice_", "").replace("_x64-setup.exe",".exe")}</span>
                    <span className="shrink-0 font-mono text-xs text-faint">{formatMB(primaryWindows.size)}</span>
                  </div>
                  <a
                    ...
                    download
                  >
                    <svg aria-hidden viewBox="0 0 24 24" fill="none" className="h-4 w-4 shrink-0" stroke="currentColor" strokeWidth="1.8">
                      <path d="M12 5v10M8 11l4 4 4-4M3 17v2a2 2 0 002 2h14a2 2 0 002-2v-2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                    Download for Windows
                  </a>
                  <details className="mt-3 rounded-xl bg-canvas ring-1 ring-line">
                    <summary className="cursor-pointer list-none px-3 py-2.5 text-center font-mono text-xs text-sub hover:text-ink">Install command</summary>
                    <div className="border-t border-line p-2.5">
                      <code className="block rounded-lg bg-raised px-3 py-2 font-mono text-xs text-sub">winget install Algorith.Voice</code>
                    </div>
                  </details>
                </>
              ) : (
                <div className="mt-4 rounded-xl border border-dashed border-line bg-canvas/50 p-4 text-center">
                  <p className="font-mono text-xs text-faint">No Windows installer in this release</p>
                </div>
              )}
            </article>
          </Reveal>
          ...
          <Reveal delay={120} className="min-w-0">
            <article className="group flex flex-col rounded-2xl border border-line bg-surface p-5 shadow-[0_8px_24px_rgba(0,0,0,0.2)] transition-all duration-200 hover:bg-raised sm:p-6">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="rounded-full border border-line bg-canvas px-2 py-1 font-mono text-[11px] font-medium text-faint">amd64 · portable</span>
              </div>
              <h3 className="t-h2 mt-3 !text-[22px] sm:!text-[24px]">Linux</h3>
              <p className="mt-1 font-mono text-xs leading-4 text-faint">Ubuntu 22.04+ · Wayland needs ydotool</p>
              {linux.length > 0 && primaryLinux ? (
                <>
                  <div className="mt-4 flex items-center justify-between gap-3 rounded-xl bg-canvas px-3 py-2.5 ring-1 ring-line">
                    <span className="min-w-0 truncate font-mono text-xs font-medium text-ink">{primaryLinux.name.replace("Algorith.Voice_","")}</span>
                    <span className="shrink-0 font-mono text-xs text-faint">{formatMB(primaryLinux.size)}</span>
                  </div>
                  <a
                  ...
                  {linux.find((a) => /\.deb$/i.test(a.name)) ? (
                    <a
                      href={linux.find((a) => /\.deb$/i.test(a.name))!.browser_download_url}
                    ...
                    >
                      .deb · {formatMB(linux.find((a) => /\.deb$/i.test(a.name))!.size)} — system install
                    </a>
                  ) : null}
                  <details className="mt-3 rounded-xl bg-canvas ring-1 ring-line">
                    <summary className="cursor-pointer list-none px-3 py-2.5 text-center font-mono text-xs text-sub hover:text-ink">Quick run</summary>
                    <div className="border-t border-line p-2.5">
                      <code className="block break-all rounded-lg bg-raised px-3 py-2 font-mono text-xs leading-5 text-sub">chmod +x *.AppImage && ./Algorith*.AppImage</code>
                    </div>
                  </details>
                </>
              ) : (
                <div className="mt-4 rounded-xl border border-dashed border-line bg-canvas/50 p-4 text-center">
                  <p className="font-mono text-xs text-faint">No Linux installers in this release</p>
                </div>
              )}
            </article>
          </Reveal>
          ...
          <Reveal delay={180} className="min-w-0">
            <article className="flex flex-col rounded-2xl border border-line bg-surface p-5 opacity-95 sm:p-6">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="rounded-full bg-amber-500/10 px-2.5 py-1 font-mono text-[11px] font-medium text-amber-300 ring-1 ring-amber-500/20">Coming soon</span>
              </div>
              <h3 className="t-h2 mt-3 !text-[22px] sm:!text-[24px]">macOS</h3>
              <p className="mt-1 font-mono text-xs leading-4 text-faint">macOS 13+ · Swift · separate bundle</p>
              <div className="mt-4 rounded-xl border border-dashed border-line bg-canvas/30 px-4 py-6 text-center">
                <p className="font-mono text-xs font-medium text-ink">No download yet</p>
                <p className="mx-auto mt-1.5 max-w-[26ch] font-mono text-xs leading-4 text-faint">Swift app in <code className="rounded bg-raised px-1 py-0.5 text-ink">apps/desktop-swift</code> — offline-first.</p>
                <Link href="/docs#macos" className="mt-3 inline-flex min-h-[36px] items-center justify-center rounded-full border border-line bg-canvas px-4 text-xs font-medium text-ink hover:bg-surface">Read macOS docs</Link>
              </div>
              <a href={`https://github.com/${repo}/releases`} target="_blank" rel="noreferrer" className="mt-3 block text-center font-mono text-xs text-faint underline decoration-line underline-offset-4 hover:text-sub">Releases →</a>
            </article>
          </Reveal>
        </div>
          ...
          <Reveal>
            <section className="rounded-[20px] border border-line bg-surface p-6 sm:p-8">
              ...
              <h3 className="t-h2 mt-2">
                ...
                {rel?.prerelease ? <span className="rounded-full bg-amber-500/15 px-2 py-1 font-mono text-xs text-amber-300">prerelease</span> : null}
              </h3>
              {rel?.body ? (
                <div className="mt-4 max-h-[320px] overflow-auto rounded-xl border border-line bg-canvas p-4">
                  <pre className="whitespace-pre-wrap break-words font-mono text-xs leading-5 text-sub">{rel.body.slice(0, 4000)}</pre>
                  {rel.body.length > 4000 ? <p className="mt-3 font-mono text-xs text-faint">… truncated — see GitHub for full notes</p> : null}
                </div>
              ) : (
              ...
              <dl className="mt-6 divide-y divide-line rounded-xl border border-line bg-canvas">
                {[
                  ...
                  <div key={r.k} className="flex items-center justify-between gap-4 px-4 py-3">
                    <dt className="font-mono text-xs font-medium uppercase tracking-wide text-faint">{r.k}</dt>
                    <dd className="text-right font-mono text-xs text-ink">{r.v}</dd>
                  </div>
                ))}
              </dl>
              <div className="mt-6 rounded-xl bg-ink p-4 text-canvas">
                <p className="font-mono text-xs font-semibold uppercase tracking-wide opacity-70">Comfort tip</p>
                <p className="mt-2 font-mono text-xs leading-5 opacity-80">
                  Pin Algorith Voice to autostart, keep <code className="rounded bg-white/10 px-1 py-0.5">Ctrl+Space</code> free, and enable clipboard mode on Wayland — paste feels instant.
                </p>
                <Link href="/docs#troubleshoot" className="mt-3 inline-flex font-mono text-xs font-medium underline underline-offset-4">
                  Troubleshooting →
                </Link>
              </div>
            </section>
          ...
          <div className="rounded-[20px] border border-line bg-gradient-to-br from-surface via-surface to-raised p-6 sm:p-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="t-body mt-2 max-w-[56ch] text-sub">Speak anywhere — terminal, IDE, browser. Release to type at the cursor.</p>
              </div>
              <div className="flex shrink-0 gap-2">
                <Link href="/docs#quickstart" className="inline-flex min-h-[44px] items-center justify-center rounded-full bg-ink px-6 text-sm font-semibold text-canvas hover:bg-white">
                  Setup guide
                </Link>
                <Link href="/pricing" className="inline-flex min-h-[44px] items-center justify-center rounded-full border border-line bg-canvas px-6 text-sm font-medium text-ink hover:bg-surface">
                  Pricing
                </Link>
              </div>
            </div>
