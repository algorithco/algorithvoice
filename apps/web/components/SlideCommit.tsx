...
export default function SlideCommit({
  ...
  const edge = useTransform([seen, anchor], ([v, a]) => (v as number) + GRIP + clamp((a as number) - (v as number), 0, TRAVEL));
  const clip = useTransform(edge, (R) => `inset(0 ${INNER - (R as number)}px 0 0 round ${gripR}px)`);
  const content = useTransform([seen, edge], ([v, R]) => `translateX(${( (v as number) + (R as number)) / 2 - INNER / 2}px)`);
  const swell = hot && !held && phase === "idle" && !reduce ? SWELL : 1;
  const shape = useTransform(x, (v) => {
    const q = 1 - Math.min(SQUASH_MAX, Math.max(0, -(v as number)) / SQUASH_DIV);
  ...
  const arrow = useTransform([seen, shown], ([v, on]) => (on as number) * clamp(1 - ((v as number) - TRAVEL * 0.55) / (TRAVEL * 0.4), 0, 1));
  const trackTransform = useTransform([shake, pulse], ([s, p]) => `translateX(${s as number}px) scale(${p as number})`);
  ...
  const goHome = (velocity: number) => {
    if (reduce) animate(x, 0, { duration: 0.2, ease: EASE_OUT as unknown as never });
  ...
  const resolve = (viaKey: boolean) => {
    ...
    if (reduce) x.set(0);
      ...
      if (!viaKey && landingDip > 0) {
        animate(pulse as unknown as never, [1, 1 - landingDip, 1] as unknown as never, { duration: 0.46, times: [0, 0.62, 1], ease: EASE_OUT as unknown as never, delay: 0.1 } as unknown as never);
      }
    }
    onDone?.();
  ...
  const reject = (reason: unknown) => {
    ...
    if (reduce) goHome(0);
      ...
      animate(shake as unknown as never, SHAKE as unknown as never, { duration: 0.45, ease: EASE_OUT as unknown as never } as unknown as never);
      homeTimer.current = window.setTimeout(() => {
        if (!grip.current) goHome(0);
      }, 300);
    }
    timer.current = window.setTimeout(() => setPhase("idle"), Math.max(holdMs, 1500));
  };
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "End") {
      ...
      e.preventDefault();
      if (grip.current) up({ pointerId: grip.current.id } as unknown as PointerEvent);
      else x.set(0);
    }
  };
  ...
  return (
    ...
    >
      <motion.div ref={trackRef} className="slide-commit__track" style={{ transform: trackTransform as unknown as string }} onPointerDown={down}>
        <motion.span className="slide-commit__label" style={{ opacity: say as unknown as string }} aria-hidden="true">
        ...
        <motion.div
          ...
          style={{ clipPath: clip as unknown as string, transform: shape as unknown as string, transformOrigin: origin as unknown as string }}
          onPointerEnter={(e) => {
            if ((e as React.PointerEvent).pointerType === "mouse" && finePointer()) setHot(true);
          }}
          onPointerLeave={() => setHot(false)}
          onKeyDown={onKeyDown}
        >
          <motion.div className="slide-commit__content" style={{ transform: content as unknown as string }}>
            <motion.span className="slide-commit__arrow" style={{ opacity: arrow as unknown as string }} aria-hidden="true">
              {icon ?? <ArrowIcon size={iconSize} />}
            </motion.span>
            <motion.span className="slide-commit__spin" style={{ opacity: spin as unknown as string }} aria-hidden="true">
