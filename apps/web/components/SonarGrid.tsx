...
// Vendored for the login background only — monochrome sonar dot field.
// Reads the resolved `text-ink` colour (white on our black login page), so no
// hue anywhere. Idles with no live ring, pauses off-screen / hidden tabs,
// still grid under reduced-motion.
export interface SonarGridProps {
  ...
  baseOpacity?: number;
  /** Any CSS colour. Defaults to the resolved text colour. */
  color?: string;
  ...
  maxRings?: number;
  /** Start with one ring already mid-expansion so the first frame reads. */
  seedPing?: boolean;
  /** Where ambient pings (and the seed ping) may spawn: [x0, y0, x1, y1]. */
...
export function SonarGrid({
  ...
  // The render loop reads props through this ref so knob changes apply live
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      for (let i = 0; i < cols; i++) {
        ...
        for (let j = 0; j < rows; j++) {
          ...
          let energy = 0;
          for (const r of live) {
            if (Math.abs(cx - r.x) > r.reach || Math.abs(cy - r.y) > r.reach)
              continue;
      ...
      for (let k = 0; k < hot.length; k += 3) {
        ...
        ctx.arc(
          hot[k] ?? 0,
          hot[k + 1] ?? 0,
          o.dotRadius * (1 + o.amplitude * energy),
          0,
          TAU,
        );
        ctx.fill();
    ...
    const resize = () => {
      ...
      if (!seeded) {
        // One ring already mid-expansion inside the ping area, so the first
        // paint shows the idea.
        seeded = true;
    ...
    refreshRef.current = () => {
      ...
      nextPing = Math.min(
        nextPing,
        performance.now() + opts.current.pingEvery * 1000,
      );
      wake();
    };
  ...
  return (
    ...
    >
      {/*
        ...
        never focusable, pointer-events-none, screen-reader hidden on purpose.
      */}
      <canvas
        ref={canvasRef}
        aria-hidden="true"
