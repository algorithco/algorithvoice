...
export default function JellyRadio({
  ...
  const at = atRaw; // allow -1 for no selection (e.g. /download)
  ...
  const _itemsKey = list.map((it) => it.value).join("|");
  ...
  const apply = (sel: number, instant: boolean) => {
    ...
    for (let i = 0; i < C.count; i++) {
      ...
      const far = sel === -1 ? 0 : Math.abs(i - sel);
      const dir = sel === -1 ? 0 : Math.sign(i - sel) * (rtl ? -1 : 1);
      const x = dir * push;
      const s = sel === -1 ? 1 : on ? 1 + C.swell : 1 - C.shrink;
      if (instant || C.reduce) {
        mv.x.jump(x);
        mv.sx.jump(s);
        mv.sy.jump(s);
  ...
  useLayoutEffect(() => {
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: stable via ref
  }, [apply, measure]);
  ...
  useEffect(() => {
    ...
    apply(at, true);
    // eslint-disable-next-line react-hooks/exhaustive-deps
    // biome-ignore lint/correctness/useExhaustiveDependencies: at derived
  }, [at, apply]);
    ...
    >
      {list.map((it, i) => (
        <Chip
          ...
          onClick={(e) => commit(i, (e as React.MouseEvent).detail === 0)}
