...
public struct SHA256Hasher: Sendable {
    ...
    private mutating func processBlock(_ block: Data) {
        ...
        for index in 16..<64 {
            let older = schedule[index - 15]
            let recent = schedule[index - 2]
            let sigmaSmall0 = Self.rotateRight(older, 7)
                ^ Self.rotateRight(older, 18)
                ^ (older >> 3)
            let sigmaSmall1 = Self.rotateRight(recent, 17)
                ^ Self.rotateRight(recent, 19)
                ^ (recent >> 10)
            schedule[index] = schedule[index - 16] &+ sigmaSmall0 &+ schedule[index - 7] &+ sigmaSmall1
        }
        // Working state as an 8-word array (avoids eight single-letter
        // locals): indices 0...7 play the FIPS roles a...h.
        var work = state
        for index in 0..<64 {
            let bigSigma1 = Self.rotateRight(work[4], 6)
                ^ Self.rotateRight(work[4], 11)
                ^ Self.rotateRight(work[4], 25)
            let choice = (work[4] & work[5]) ^ (~work[4] & work[6])
            let tempFirst = work[7] &+ bigSigma1 &+ choice &+ Self.roundConstants[index] &+ schedule[index]
            let bigSigma0 = Self.rotateRight(work[0], 2)
                ^ Self.rotateRight(work[0], 13)
                ^ Self.rotateRight(work[0], 22)
            let majority = (work[0] & work[1]) ^ (work[0] & work[2]) ^ (work[1] & work[2])
            let tempSecond = bigSigma0 &+ majority
            work[7] = work[6]
            work[6] = work[5]
            work[5] = work[4]
            work[4] = work[3] &+ tempFirst
            work[3] = work[2]
            work[2] = work[1]
            work[1] = work[0]
            work[0] = tempFirst &+ tempSecond
        }
        for index in 0..<8 {
