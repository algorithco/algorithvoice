...
fn enforce_signed_manifest() {
    ...
    // Structural check only: a top-level non-empty hex "signature" string.
    // A cheap textual scan avoids adding serde_json to build-dependencies.
    // Cryptographic validity is proven by the
    // `bundled_manifest_verifies_against_baked_pubkey` unit test.
    ...
    if !has_signature {
        panic!(
            "BUILD REJECTED: src/local_asr/default_manifest.json carries no \
             model-manifest signature. Re-sign it with \
             `python3 scripts/sign-manifest.py sign --key <seed> \
             --manifest src/local_asr/default_manifest.json` before building."
        );
    }
